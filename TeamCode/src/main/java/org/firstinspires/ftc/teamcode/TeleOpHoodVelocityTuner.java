...
public class TeleOpHoodVelocityTuner extends LinearOpMode {
    ...
    //
    //  State 0 — Start flywheels and wait for them to rev up.
    //  State 1 — Shift sorter one slot forward.
    //  State 2 — Wait for sorter to finish.
    //  State 3 — Settle 0.35 s, then extend kicker.
    //  State 4 — Wait 0.75 s, retract kicker.
    //  State 5 — Wait 1.0 s, stop flywheels, mark done.
    //
    public static double SPINUP_WAIT = 2.5; // seconds — tune from dashboard
    ...
    private void shootingSequence() {
        ...
        if (rb && !lastRB && !shootingActive) {
            shootingActive = true;
            shootState     = 0;
            sorterTimer    = getRuntime(); // reuse sorterTimer for spin-up wait
        }
        lastRB = rb;
        ...
        switch (shootState) {
            ...
            case 0:
                // Wait for flywheels to reach speed before moving anything
                if (getRuntime() - sorterTimer >= SPINUP_WAIT) {
                    shootState = 1;
                }
                break;
    ...
    private void updateTelemetry() {
        ...
        telemetryDashboard.addData("SPINUP_WAIT (s)",     SPINUP_WAIT);
